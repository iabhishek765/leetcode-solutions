...
# Difficulty: Hard
# Topics: String, Greedy, Binary Search, Run-Length Encoding
#
# Approach: Build run-length encoding on augmented string.
#           For each query [l,r], find 0-blocks fully inside
#           range with 1-block neighbors (clipped to range).
#           Best gain = max(zeros - min(left_ones, right_ones))
# Time: O(n + Q*k) | Space: O(n)
#
# ML Connection: Range queries on encoded sequences mirror
# attention masking in transformers where only specific
# token ranges are attended to per query head
